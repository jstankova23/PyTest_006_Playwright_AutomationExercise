...
# TEST: OOBJEDNÁVKA 15. A 16. PRODUKTU S REGISTRACÍ UŽIVATELE 'Test_24 User', KONTROLA OBJEDNÁVKY A ADRES, VLOŽENÍ KOMENTÁŘE, PLATBA, STAŽENÍ FAKTURY A SMAZÁNÍ UŽIVATELE
# Test ověřuje dočasnou hlášku v DOM (success alert) po potvrzení platby a stažení souboru s fakturou.

def test_order_reg_pay_inv(page: Page):
    # 1. Launch browser; 
    ...
    ### Přidání 15. a 16. produktu dle pořadí zobrazení na stránce do košíku, identifikace konkrétních produktů probíhá ale podle jejich ID v DOM (ID neodpovídá vždy pořadí na stránce).   
    ...
    ### Přidání 15. produktu do košíku
    ###### a) Vyhledání 15. produktu dle ID přes href v mřížce a hover
    product_id = "18"                       # POZOR: pořadové číslo produktu viditelné z aplikace neodpovídá ID produktu v DOM
    product_15 = features_products.filter(has=page.locator(f"a[data-product-id='{product_id}']")).first # vyhledání první karty produktu z mřížky (products), která obsahuje odkaz na detail produktu s daným ID
    product_15.scroll_into_view_if_needed() # pokud je karta produktu mimo viditelnou část stránky, Playwright ji posune do zorného pole, overlay se často aktivuje jen na viditelné kartě
    product_15.hover()                      # simulace najetí myší na kartu produktu, tím se zobrazí overlay vrstva (.product-overlay) a v ní tlačítko 'Add to cart'

    ###### b) Overlay vrstva: Kliknutí na tlačítko 'Add to Cart'
    add_to_cart_prod15_btn = product_15.locator(".overlay-content .btn") # vyhledání tlačítka 'Add to Cart' v overlay vrstvě (vnořený lokátor) UVNITŘ TÉTO KONKRÉTNÍ KARTY produktu 
    add_to_cart_prod15_btn.wait_for(state="visible") # vyčkání, až se overlay vrstva skutečně ukáže
    add_to_cart_prod15_btn.click(force=True)         # kliknutí na tlačítko 'Add to Cart', i když ho dočasně něco překrývá, klik i při krátkém překrytí karty produktu
    ...
    ### Přidání 16. produktu do košíku
    ###### a) Vyhledání 16. produktu dle ID přes href v gridu a hover
    product_id = "19"                       # POZOR: pořadové číslo produktu viditelné z aplikace neodpovídá ID produktu v DOM
    product_16 = features_products.filter(has=page.locator(f"a[data-product-id='{product_id}']")).first 
    product_16.scroll_into_view_if_needed()  
    product_16.hover()

    ###### b) Overlay vrstva: Kliknutí na tlačítko 'Add to Cart'
    add_to_cart_prod16_btn = product_16.locator(".overlay-content .btn")
    add_to_cart_prod16_btn.wait_for(state="visible") 
    add_to_cart_prod16_btn.click(force=True)
    ...
    name = "Test_24 User" # toto jméno se nedotahuje do adres+ v bodu f) se zadávají hodnoty do polí 'Firt Name', 'Last Name' a tyto hodnoty se dotahují do adres 
    email = f"test_24_user_{uuid.uuid4().hex[:8]}@example.com" # dynamické vygenerování unikátního emailu
    ...
    ###### Datum narození: 10. července 1964 (nepovinné)
    page.select_option("#days", "10")        
    page.select_option("#months", "July")      
    page.select_option("#years", "1964")     
    ...
    address2 = "Apt. 12B"           # nepovinné
    state = "Auckland"
    city = "Auckland"
    zip_code = "1010"
    mobile_num = "+64211234567"
    ...
    ### Kontrola pouze přítomnosti produktů 15 a 16 v objednávce
    ...
    product_15_row = order_table.locator("tr#product-18")   # lokátor pro řádek produktu 15 vnořený do lokátoru pro tabulku objednávky, POZOR: ID neodpovídá pořadovému čislu v UI
    product_16_row = order_table.locator("tr#product-19")   # lokátor pro řádek produktu 16 vnořený do lokátoru pro tabulku objednávky, POZOR: ID neodpovídá pořadovému čislu v UI

    ### Ověření, že jsou oba produkty skutečně přítomny v objednávce
    expect(product_15_row).to_be_visible()
    expect(product_16_row).to_be_visible()
    ...
    comment_text_area.fill("Test_24 User - test comment.")
    ...
    name_on_card = "Test_24 User"
    card_number = "555 6666 7777 8888"
    cvc = "999"
    mm = "11"
    yyyy = "2027"
